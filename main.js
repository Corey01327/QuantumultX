...
function handleBasicConfig() {
    ...
    $.log("Raw response body: " + resp.body);

    let body;
    try {
        body = JSON.parse(resp.body);
        $.log("Parsed response body successfully.");
    } catch (e) {
        $.log("Error parsing response body: " + e.message);
        // Early exit if parsing fails
        $.done();
        return;
    }

    if (body && body.data && body.data.JDHttpToolKit) {
        $.log("JDHttpToolKit found in response body.");

        // Log the original JDHttpToolKit object
        $.log("Original JDHttpToolKit: " + JSON.stringify(body.data.JDHttpToolKit));

        if (body.data.JDHttpToolKit.httpdns) {
            delete body.data.JDHttpToolKit.httpdns;
            $.log("Deleted JDHttpToolKit.httpdns.");
        } else {
            $.log("JDHttpToolKit.httpdns not found or already deleted.");
        }

        if (body.data.JDHttpToolKit.dnsvipV6) {
            delete body.data.JDHttpToolKit.dnsvipV6;
            $.log("Deleted JDHttpToolKit.dnsvipV6.");
        } else {
            $.log("JDHttpToolKit.dnsvipV6 not found or already deleted.");
        }
    } else {
        $.log("JDHttpToolKit not found in response body.");
    }

    // Log the modified response body
    $.log("Modified response body: " + JSON.stringify(body));
